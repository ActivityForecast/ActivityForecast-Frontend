...
export default function HistoryPage() {
  ...
  // 임시 타임라인 데이터 (해당 월)
  const timeline = [
    { id: 1, title: "축구", date: "2025년 10월 17일", done: true, rating: 5, img: null, difficulty: "상" },
    { id: 2, title: "러닝", date: "2025년 10월 13일", done: true, rating: 5, img: null, difficulty: "중" },
    { id: 3, title: "농구", date: "2025년 10월 8일", done: false, rating: 3.5, img: null, difficulty: "중" },
    { id: 4, title: "축구", date: "2025년 10월 3일", done: true, rating: 4, img: null, difficulty: "상" },
  ];
  ...
  const extractMonth = (d) => {
    ...
    return m ? parseInt(m[1], 10) : thisMonth;
  };

  const monthLabel = `${selectedMonth}월 타임라인`;
  const displayedTimeline = useMemo(
    () => timeline.filter((t) => extractMonth(t.date) === selectedMonth),
  ...
  useEffect(() => {
    ...
    const spacer = Math.max(0, Math.round(el.clientHeight / 2 - monthItemHeight / 2));
  ...
  return (
    <main className="min-h-screen bg-[#F8FAFC] flex items-center justify-center relative">
      ...
      >
        ...
        <section className="w-full mt-8">
          <div className="rounded-3xl bg-[#F8FAFC]  p-4 sm:p-6 md:p-8">
            ...
            <div className="rounded-2xl shadow-md">
              <ActivityWidget accent="#3B82F6" total={total} segments={segments} withBorder={false} />
            </div>
          </div>
        </section>
          ...
          <div className="relative rounded-3xl bg-white shadow-sm px-6 py-6">
            <div className="text-center text-lg font-semibold mb-6">
              ...
              <button
                ...
                onClick={() => { setDraftMonth(selectedMonth); setOpenMonthModal(true); }}
              >
                📅
              </button>
            </div>
            ...
            >
              {displayedTimeline.length === 0 && (
                <div className="w-full text-center text-sm text-gray-500 py-6">해당 월의 기록이 없습니다</div>
              )}
              {displayedTimeline.map((item) => (
                ...
                >
                  ...
                  <div className="mt-2 text-base font-semibold text-center">{item.title}</div>
                  <div className="text-[11px] text-gray-500 mt-1 text-center">{item.date}</div>
                  <div className="mt-2 text-[12px] text-gray-700 text-center">★ / {item.rating}</div>
                </button>
              ))}
            </div>
          </div>
        ...
        {/* 월 선택 모달 */}
        <Modal isOpen={openMonthModal} onClose={() => setOpenMonthModal(false)} title="월 선택">
          <div className="mt-2">
              ...
              >
                {Array.from({ length: 12 }, (_, i) => i + 1).map((m) => (
                  <button
                    key={m}
                    type="button"
                    onClick={() => { setDraftMonth(m); scrollToMonth(m); }}
            ...
            <ModalFooter>
              <button
                ...
                onClick={() => { setSelectedMonth(draftMonth); setOpenMonthModal(false); }}
              >
                확인
              </button>
            </ModalFooter>
        ...
        <div style={{ '--modal-w-sm': '960px', '--modal-min-h': '400px' }}>
          <Modal isOpen={detailOpen} onClose={closeDetail} title={detailItem?.title || ""}>
            ...
            <div className="flex-shrink-0 w-[220px]">
              <div
                ...
                style={{
                  backgroundImage: detailItem?.img ? `url(${detailItem.img})` : undefined,
                  backgroundSize: "cover",
                  backgroundPosition: "center",
                }}
              />
              <div className="mt-3 text-center text-gray-800 font-semibold">
                난이도 {detailItem?.difficulty || (detailItem?.title === '축구' ? '상' : detailItem?.title === '러닝' ? '중' : '중')}
              </div>
            </div>
            ...
            <div className="flex-1">
              <div className="relative rounded-3xl bg-white border border-gray-100 p-6">
                ...
                <div className="space-y-5">
                  <div>
                    <div className="text-sm font-semibold text-gray-700">운동일</div>
                    <div className="mt-1 text-gray-800">{detailItem?.date}</div>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="mt-1 flex items-center gap-2">
                      ...
                      <span className="text-[11px] rounded-full bg-[#DBEAFE] text-[#3B82F6] px-2 py-[2px]">자세한 위치를 적어주세요</span>
                    </div>
                  </div>

                  <div className="flex items-center gap-3">
                    <div className="text-sm font-semibold text-gray-700">활동유무</div>
                    <span className={`inline-block h-3.5 w-3.5 rounded-full ${detailItem?.done ? "bg-[#22C55E]" : "bg-[#EF4444]"}`} />
                  </div>

                  <div className="flex items-center gap-3">
                    <div className="text-sm font-semibold text-gray-700">평점</div>
                    <select defaultValue={detailItem?.rating || 5} className="border rounded-md px-2 py-1 text-sm">
              ...
              <ModalFooter>
                <button type="button" className="flex-1 rounded-xl bg-[#3B82F6] text-white py-3" onClick={closeDetail}>
                  저장하기
                </button>
              </ModalFooter>
            </div>
