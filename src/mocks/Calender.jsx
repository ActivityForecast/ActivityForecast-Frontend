...
export default function CalendarBox({
  ...
  const matrix = useMemo(() => {
    ...
    for(let i=0;i<startIdx;i++){
      const d = new Date(first); d.setDate(first.getDate()-(startIdx-i));
    ...
    while(days.length%7!==0) {
      const d = new Date(days[days.length-1].d); d.setDate(d.getDate()+1);
      days.push({ d, other:true });
    }
    // 6주 보장
  ...
  const dowClass = isMini ? "text-xs py-1" : isXl ? "text-lg py-4" : isLg ? "text-base py-3" : "text-sm py-2";
  ...
  const cellW = isMini
    ...
    ? (wide ? "w-16" : "w-12")
    : (wide ? "w-14" : "w-10");
  const numClass = isMini ? "text-xs" : isXl ? "text-xl" : isLg ? "text-lg" : "text-base";
  const navBtnSize = isMini ? "w-8 h-8" : isXl ? "w-12 h-12" : isLg ? "w-10 h-10" : "w-8 h-8";
  const navIconSize = isMini ? "w-4 h-4" : isXl ? "w-6 h-6" : isLg ? "w-5 h-5" : "w-4 h-4";
  const headerText = isMini ? "text-sm font-semibold" : isXl ? "text-3xl font-bold" : isLg ? "text-2xl font-bold" : "text-lg font-bold";

  return (
    <div className={`${inline ? "relative" : "fixed"} bg-white ${wrapCls} ${cardPad} ${className}`}>
      {/* 헤더 */}
      ...
      <div className="grid grid-cols-7 mt-2">
        {["일","월","화","수","목","금","토"].map(d => (
          <div key={d} className={`text-center text-gray-500 ${dowClass}`}>{d}</div>
        ))}
      </div>
            ...
            <div key={idx} className="flex items-center justify-center py-1">
              <div
                className={`grid place-items-center rounded-full border ${cellH} ${cellW} ${numClass}
                  ${other ? "text-gray-300 border-transparent" : "text-black border-transparent"}
                  hover:bg-gray-50`}
                style={ hasEvt ? { borderColor: `${accent}55`, color: "#000" } : undefined }
                title={hasEvt ? eventMap.get(k).map(e=>e.label).join(", ") : undefined}
              >
                {d.getDate()}
                {/* 이벤트 점 */}
                {hasEvt && (
                  <span className="mt-0.5 block h-1.5 w-1.5 rounded-full" style={{ backgroundColor: accent }} />
                )}
              </div>
            </div>
          );
